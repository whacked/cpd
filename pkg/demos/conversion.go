...
func parseYamlWithOrder(filename string) (map[string]interface{}, []string, error) {
	...
	if err := goyaml.Unmarshal(data, &result); err != nil {
...
func MetaVersionDemo() {
	...
	reader := yaml.NewReader(bytes.NewReader(source))
	...
	for i, doc := range docs {
		...
		yaml.DebugPrint = true
		doc.PrintDebug()
		yaml.DebugPrint = false
	...
	// Demo order-preserving JSON parsing
	fmt.Printf("\n\n=== Order-Preserving JSON Parsing Demo ===\n")

	// Test cases with intentionally confusing key order
	testCases := []string{
		// Case 1: Mixed case keys
		`{"zebra":1,"apple":2,"Banana":3,"cherry":4}`,

		// Case 2: Numeric-looking keys
		`{"2":1,"1":2,"10":3,"3":4}`,

		// Case 3: Special characters in keys
		`{"key-1":1,"key_2":2,"key.3":3,"key:4":4}`,

		// Case 4: Nested objects
		`{"outer":{"z":1,"a":2},"inner":{"y":3,"b":4}}`,

		// Case 5: Mixed types of values
		`{"string":"value","number":42,"array":[1,2,3],"object":{"x":1,"y":2}}`,
	}
	...
	for i, testCase := range testCases {
		...
		// Parse using standard JSON (order not preserved)
		var standardMap map[string]interface{}
		if err := json.Unmarshal([]byte(testCase), &standardMap); err != nil {
			log.Printf("Standard JSON parse error: %v", err)
			continue
		}
		fmt.Printf("Standard JSON (keys may be reordered):\n")
		for k, v := range standardMap {
			fmt.Printf("  %s: %v\n", k, v)
		}

		// Parse using ordered map (order preserved)
		orderedMap := orderedmapjson.NewAnyOrderedMap()
		if err := json.Unmarshal([]byte(testCase), orderedMap); err != nil {
			log.Printf("Ordered JSON parse error: %v", err)
			continue
		}
		fmt.Printf("Ordered JSON (original order preserved):\n")
